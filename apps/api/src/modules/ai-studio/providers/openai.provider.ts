...
export class OpenAIProvider implements AIProvider {
      ...
      const req = https.request(reqOptions, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            if (parsed.error) {
              return reject(new Error(`OpenAI error: ${parsed.error.message}`));
            }
            resolve(parsed.choices?.[0]?.message?.content ?? '');
          } catch (e) {
            reject(new Error(`OpenAI parse error: ${String(e)}`));
          }
        });
      });
  ...
  async generateStructuredJSON<T = Record<string, unknown>>(
    ...
    try {
      ...
      throw new Error(`OpenAI did not return valid JSON. Response: ${text.substring(0, 200)}`);
    }
  }
}
