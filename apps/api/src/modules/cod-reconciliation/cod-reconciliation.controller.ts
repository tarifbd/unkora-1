import {
  ...
  Param, Patch, Query, UseGuards, Version,
} from '@nestjs/common';
...
export class CodReconciliationController {
  ...
  findAll(
    ...
    return this.codService.findAll({ provider, reconciled, page: +page, limit: +limit });
  }
  ...
  @Patch(':id/reconcile')
  @Version('1')
  @ApiOperation({ summary: 'Mark COD as received/reconciled' })
  reconcile(
    @Param('id') id: string,
    @Body() dto: { codNotes?: string },
    @CurrentUser() user: { id: string },
  ) {
    return this.codService.reconcile(id, dto, user.id);
  }

  @Patch(':id/unreconcile')
  @Version('1')
  @ApiOperation({ summary: 'Undo COD reconciliation' })
  unreconcile(@Param('id') id: string) {
    return this.codService.unreconcile(id);
  }
}
