...
export default function RbacPage() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      api.get('/staff/stats').then(r => r.data.data).catch(() => ({ totalStaff: 0, pendingInvitations: 0 })),
  });
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
          <UserCog className="h-5 w-5 text-orange-500" /> RBAC — Role-Based Access Control
        </h1>
        <p className="text-sm text-gray-500 mt-1">Manage roles and permissions for the UNKORA platform.</p>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
        ...
        <p className="text-sm text-amber-700 mt-1">
          Invite new staff from the{' '}
          <a href="/admin/staff" className="underline font-semibold text-amber-800">Staff &amp; Permissions</a>{' '}
          page. Only SUPER_ADMIN can promote users to ADMIN or SUPER_ADMIN roles.
        </p>
      </div>
    </div>
  );
