  ...
  return (
    ...
    >
      ...
      <div className="p-2.5 sm:p-3 flex flex-col flex-grow">
        {/* Category tag */}
        {product.category && (
          <span className="text-[9px] sm:text-[10px] font-bold text-primary/70 uppercase tracking-wider mb-1">
            {product.category.name}
          </span>
        )}

        <h3 className="text-xs sm:text-sm font-bold text-gray-800 line-clamp-2 mb-1 group-hover:text-primary transition-colors leading-4 sm:leading-5 flex-grow">
          {product.name}
        </h3>

        {product.bookDetail?.author && (
          <p className="text-[10px] sm:text-xs text-gray-400 mb-1.5 truncate">{product.bookDetail.author}</p>
        )}
          ...
          <div className="flex items-center gap-1 mb-1.5">
            <div className="flex">
              {Array.from({ length: 5 }).map((_, i) => (
                <svg key={i} className={cn('w-2.5 h-2.5 sm:w-3 sm:h-3', i < fullStars ? 'text-yellow-400' : 'text-gray-200')} fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              ))}
            </div>
            <span className="text-[9px] sm:text-[10px] text-gray-400">({reviewCount})</span>
          </div>
        )}

        {/* Price */}
        <div className="flex items-baseline gap-1.5 mb-2 sm:mb-3">
          <span className="text-sm sm:text-base font-black text-primary">৳{price.toLocaleString('en-BD')}</span>
          {hasDiscount && (
            <span className="text-[10px] sm:text-xs text-gray-400 line-through">৳{Number(product.basePrice).toLocaleString('en-BD')}</span>
          )}
        </div>
        ...
        {product.stockQuantity > 0 ? (
          <button
            onClick={handleAddToCart}
            disabled={addItem.isPending}
            className="w-full flex items-center justify-center gap-1.5 py-2 sm:py-2.5 bg-primary/5 border border-primary/20 rounded-lg text-xs sm:text-sm font-bold text-primary hover:bg-primary hover:text-white hover:border-primary transition-all duration-300 disabled:opacity-50"
          >
            <ShoppingCart className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
            <span className="truncate">{t.product.addToCart}</span>
          </button>
        ) : (
          <button disabled className="w-full py-2 sm:py-2.5 bg-gray-100 rounded-lg text-xs sm:text-sm font-medium text-gray-400 cursor-not-allowed">
            {t.product.outOfStock}
          </button>
        )}
        {product.stockQuantity > 0 && (
          <Link
            ...
            onClick={e => e.stopPropagation()}
            className="w-full flex items-center justify-center gap-1.5 py-1.5 sm:py-2 bg-secondary text-white rounded-lg text-[10px] sm:text-xs font-bold hover:bg-secondary/90 transition-all mt-1.5"
          >
