...
export function ContactWidget() {
  const { data: config } = useQuery<Record<string, string>>({
    ...
    queryFn: () =>
      api.get('/chatbot/config').then(r => (r.data?.data ?? {}) as Record<string, string>),
    staleTime: 5 * 60_000,
    retry: false,
  });
  ...
  if (!config || (!whatsapp && !messenger && !botEnabled)) return null;
    ...
    try {
      ...
      setMessages(prev => [
        ...prev,
        {
          role: 'assistant',
          content:
            'দুঃখিত, কিছু একটা সমস্যা হয়েছে। Sorry, something went wrong — please try again or reach us on WhatsApp.',
        },
      ]);
    } finally {
      setSending(false);
  ...
  return (
    <>
      <div className="fixed right-3 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-3">
        ...
        {botEnabled && (
          <button
            type="button"
            onClick={() => setOpen(o => !o)}
            aria-label="Chat with Unkora AI"
            className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-orange-400 to-orange-600 text-white shadow-lg transition-transform hover:scale-110"
          >
      ...
      {open && botEnabled && (
        <div className="fixed bottom-4 right-4 z-50 flex w-[min(380px,calc(100vw-2rem))] h-[520px] flex-col overflow-hidden rounded-2xl border bg-white shadow-2xl">
          <div className="flex items-start justify-between bg-gradient-to-r from-orange-500 to-orange-600 px-4 py-3 text-white">
            <div>
              <div className="flex items-center gap-2">
                <Sparkles className="h-5 w-5" />
                <h3 className="font-semibold">Unkora AI</h3>
              </div>
          ...
          <div className="flex items-center gap-2 border-t p-3">
            ...
            <button
              ...
              className="flex h-9 w-9 items-center justify-center rounded-full bg-orange-500 text-white transition-colors hover:bg-orange-600 disabled:opacity-40"
            >
